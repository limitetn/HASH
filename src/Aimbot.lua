...
--// Environment
--// Real Silent Aim Variables
local RealSilentAim = false
	...
	ServiceConnections.RenderSteppedConnection = Connect(__index(RunService, Environment.DeveloperSettings.UpdateMode), function()
		...
		if Running and Settings.Enabled then
			...
			if Environment.Locked then
				...
				if RealSilentAim then
					-- Use real silent aim - modify camera without moving mouse
					self:ApplySilentAim(Environment.Locked)
				elseif Environment.Settings.LockMode == 2 then
...
--// Functions
-- Real Silent Aim Functions
function Environment.SetRealSilentAim(self, enabled)
	RealSilentAim = enabled
end

function Environment.GetRealSilentAimTarget(self)
	return SilentAimTarget
end

function Environment.ApplySilentAim(self, target)
	if not RealSilentAim then return end
	
	SilentAimTarget = target
	-- Implementation for real silent aim
	-- This modifies the camera position silently without moving the mouse
	if target and target.Character then
		...
		if targetPart then
			...
			-- Set camera to look at target silently
			Camera.CFrame = CFrame.new(Camera.CFrame.Position, targetPart.Position)
		end
	end
end

function Environment.ResetSilentAim(self)
	if SilentAimTarget then
		SilentAimTarget = nil
		-- Reset any silent aim modifications
