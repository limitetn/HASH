...
-- Animation functions
function utility.tween(object, property, targetValue, duration, callback)
	local startTime = tick()
	local startValue = object[property]
	local range = targetValue - startValue
	...
	local function update()
		...
		object[property] = startValue + (range * ease)
...
function utility.animateColor(object, targetColor, duration, callback)
	local startTime = tick()
	local startColor = object.Color
	local rRange = targetColor.R - startColor.R
	local gRange = targetColor.G - startColor.G
	local bRange = targetColor.B - startColor.B
	...
	local function update()
		...
		object.Color = Color3.new(
			startColor.R + (rRange * ease),
			startColor.G + (gRange * ease),
			startColor.B + (bRange * ease)
		)
...
function utility.fade(object, targetTransparency, duration, callback)
	local startTime = tick()
	local startTransparency = object.Transparency
	local range = targetTransparency - startTransparency
	...
	local function update()
		...
		object.Transparency = startTransparency + (range * ease)
